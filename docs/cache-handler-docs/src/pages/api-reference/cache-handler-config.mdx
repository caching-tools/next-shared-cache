...
If a handler is `undefined` or `null`, it will be ignored. It may be useful when you don't want to use a particular cache layer in a specific case. For example, you may want to [opt out of using your remote cache during the build](/usage/opt-out-cache-on-build).
