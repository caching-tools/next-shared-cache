The `onCreation` static method of the `CacheHandler` class is designed to facilitate the creation of a custom cache instance with configurable behavior. It accepts a single argument, `context`, which provides contextual information necessary for configuring your custom cache.
...
The `context` argument is an object of type `CacheCreationContext`, providing essential information for cache configuration. It includes the following properties:
...
In this example, the `onCreation` method configures two Handlers, and common TTL policy for the cache.
