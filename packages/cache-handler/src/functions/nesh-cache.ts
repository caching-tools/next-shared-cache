...
>(
  ...
  if (commonOptions?.resultSerializer && !commonOptions?.resultDeserializer) {
    throw new Error(
      'neshCache: if you provide a resultSerializer, you must provide a resultDeserializer.',
    );
  }

  if (commonOptions?.resultDeserializer && !commonOptions?.resultSerializer) {
    throw new Error(
      'neshCache: if you provide a resultDeserializer, you must provide a resultSerializer.',
    );
  }
  ...
  async function cachedCallback(
    ...
    assert(
      ...
      'neshCache must be used in a Next.js app directory.',
    );
    ...
    for (const tag of combinedTags) {
      if (typeof tag === 'string') {
        ...
        console.warn(
          `neshCache: Invalid tag: ${tag}. Skipping it. Expected a string.`,
        );
      }
    }
