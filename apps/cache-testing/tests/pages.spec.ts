...
test.describe('On-demand revalidation', () => {
  for (const path of paths) {
    test(`If revalidate is clicked, then page should be fresh after reload ${path}`, async ({
      page,
      baseURL,
    }) => {
      ...
      await expect(page.getByTestId('is-revalidated-by-path')).toContainText(
        'Revalidated at',
      );
  ...
  for (const path of paths) {
    test(`If revalidate is clicked on page A, then page B should be fresh on load ${path}`, async ({
      ...
      await expect(appA.getByTestId('is-revalidated-by-path')).toContainText(
        'Revalidated at',
      );
      ...
      const valueFromPageA = Number.parseInt(
        (await appA.getByTestId('data').innerText()).valueOf(),
        10,
      );

      const valueFromPageB = Number.parseInt(
...
test.describe('Time-based revalidation', () => {
  ...
  for (const path of paths) {
    test(`Page should be fresh after reloaded once if becoming stale and waiting the same time for cache expiration ${path}`, async ({
      ...
      test.fail(
        path.includes('fallback-false'),
        'See https://github.com/vercel/next.js/issues/58094',
      );
  ...
  for (const path of paths) {
    test(`If page A is stale, then page B should be fresh after load and reload ${path}`, async ({
      ...
      await expect(appA.getByTestId('cache-state')).toContainText('stale', {
        timeout: 7500,
      });
      ...
      await expect(appB.getByTestId('cache-state')).toContainText('stale', {
        timeout: 7500,
      });
...
test.describe('Data consistency between two app instances for the same page', () => {
  for (const path of paths) {
    test(`Should be maintained when loaded at the same time ${path}`, async ({
      context,
      baseURL,
    }) => {
      const appAUrl = new URL(path, `${baseURL}:3000`);
...
test.describe('Data consistency after server restart', () => {
  for (const path of paths) {
    test(`Should be maintained between App A and App B after App B server restarts ${path}`, async ({
      ...
      let valueFromPageA = Number.parseInt(
        (await appA.getByTestId('data').innerText()).valueOf(),
        10,
      );
      ...
      const restartResult = await fetch(
        `${baseURL}:9000/restart/${appBUrl.port}`,
        {
          next: {
      ...
      valueFromPageA = Number.parseInt(
        (await appA.getByTestId('data').innerText()).valueOf(),
        10,
      );
...
test.describe('SSR', () => {
  test('verify initial cache state is fresh on SSR page load', async ({
    page,
    baseURL,
  }) => {
  ...
  test('ensure data value increments by 1 on SSR page reload', async ({
    page,
    baseURL,
  }) => {
    const url = new URL('/pages/no-paths/ssr/200', `${baseURL}:3000`);
...
test.describe('SSR neshClassicCache', () => {
  ...
  test('If revalidate is called, then page should be fresh after reload', async ({
    page,
    baseURL,
  }) => {
    const url = new URL(path, `${baseURL}:3000`);
  ...
  test('Page should be fresh after becoming stale and reloaded twice', async ({
    page,
    baseURL,
  }) => {
    const url = new URL(path, `${baseURL}:3000`);
...
test.describe('API route neshClassicCache', () => {
  ...
  test('If revalidate is called, then page should be fresh after reload', async ({
    ...
    const revalidateByApiResponse = await revalidateByApi(
      path,
      `${baseURL}:3000`,
    );
  ...
  test('If revalidate is called on page A, then page B should be fresh on load and value must be updated', async ({
    ...
    const revalidateAfter = Number.parseInt(
      appAJson.props.revalidateAfterMs,
      10,
    );
